...
// used by our FlatList to display a Planet data item
export default function ListItems({ itemData, navigatorRef, user_id }) {
const Unchecked = () => {
    return(
...
const Checked = () => {
    return(
    <View style={styles.checkbox}>
        <Icon name="checkmark-outline" size={20} style={{fontWeight:"800"}} />
    </View>);
};
    // console.log(user_id);
    ...
    return (
                    <TouchableOpacity
                        ...
                        <Image source={{ uri: itemData.image_url }}
...
function addCuisine(user_id, item_id) {
    ...
    fetch("https://fonder.edwardlin.ca/api/v1/users/addcuisine.php", requestOptions)
...
function removeCuisine(user_id, item_id) {
    ...
    fetch("https://fonder.edwardlin.ca/api/v1/users/delcuisine.php", requestOptions)
        .then(response => response.text())
        .then(result => console.log(result))
        .catch(error => console.log('error', error));
}
